 ...
 * @since 0.2.9
 */
class Qiupid_Layout_Builder_Frontend_V2  extends Qiupid_Abstract_Layout_Frontend {
	...
	public function render_item( $item_id, $col_id, $row_id, $device ) {
		 ...
		 * @since 0.2.1
		 */
		do_action( 'qiupid/builder/' . $this->id . '/before-item/' . $item_id );
		$object_item = Qiupid_Customize_Layout_Builder()->get_builder_item( $this->id, $item_id );
		...
		if ( ! $has_cb ) {
			...
			$fn            = 'qiupid_builder_' . $id . '_item';

			if ( function_exists( $fn ) ) {
				$return_render = call_user_func_array( $fn, array( $item_config, $item ) );
				$has_cb        = true;
			} else {
				$fn = 'qiupid_builder_' . $this->id . '_' . $id . '_item';
				if ( function_exists( $fn ) ) {
					$return_render = call_user_func_array( $fn, array( $item_config, $item ) );
					$has_cb        = true;
				}
		 ...
		 * @since 0.2.1
		 */
		do_action( 'qiupid/builder/' . $this->id . '/after-item/' . $item_id );
				...
				foreach ( $col_items as $item_index => $col_item ) {
					...
					if ( $item ) {
						...
						if ( $content ) {
							...
							$row_items_html_escaped .= '<div class="' . esc_attr( $item_classes ) . '" data-section="' . esc_attr($item_config['section']) . '" data-item-id="' . esc_attr( $item_id ) . '" >';
							$row_items_html_escaped .= $this->setup_item_content( $content, $id, $device );
							if ( is_customize_preview() ) {
								$row_items_html_escaped .= '<span class="item--preview-name">' . esc_html( $item_config['name'] ) . '</span>';
							}
							$row_items_html_escaped .= '</div>';
							echo $row_items_html_escaped; // WPCS: XSS ok.
						}
					}
				}
				echo '</div>';
	...
	public function render( $row_ids = array( 'top', 'main', 'bottom' ) ) {
		...
		foreach ( $row_ids as $row_id ) {
			$show = qiupid_is_builder_row_display( $this->id, $row_id );
			if ( $show && isset( $this->rows[ $row_id ] ) ) {
				...
				if ( ! empty( $show_on_devices ) ) {
					...
					if ( ! empty( $desktop_row ) || ! empty( $mobile_row ) ) {
						...
						$align_classes = 'qiupid-grid-middle';
						...
						$row_layout    = Qiupid()->get_setting( $this->id . '_' . $row_id . '_layout' );
						$row_text_mode = Qiupid()->get_setting( $this->id . '_' . $row_id . '_text_mode' );
						if ( $row_layout ) {
							$classes[] = sanitize_text_field( $row_layout );
						}
						$show_on_sticky    = Qiupid()->get_setting( $this->id . '_' . $row_id . '_sticky_status' );
						if (!$show_on_sticky) {
							$classes[] = 'qiupid-hide-on-sticky';
						}

						$classes = apply_filters( 'qiupid/builder/row-classes', $classes, $row_id, $this );

						$atts['class']       = join( ' ', $classes );
						$atts['id']          = 'cb-row--' . $_id;
						$atts['data-row-id'] = $row_id;
						$atts                = apply_filters( 'qiupid/builder/row-attrs', $atts, $row_id, $this );
						$string_atts         = '';
						foreach ( $atts as $k => $s ) {
							if ( is_array( $s ) ) {
								$s = wp_json_encode( $s );
						...
						if ( $desktop_row ) {
							$html_desktop_escaped = $this->render_row( $desktop_row, $row_id, 'desktop' );
						} else {
							$html_desktop_escaped = false;
						}
						if ( $mobile_row ) {
							$html_mobile_escaped = $this->render_row( $mobile_row, $row_id, 'mobile' );
						} else {
						...
						$inner_class = apply_filters( 'qiupid/builder/inner-row-classes', $inner_class, $row_id, $this );

						if ( $html_mobile_escaped || $html_desktop_escaped ) {
							?>
							<div <?php echo wp_kses_post($string_atts); // WPCS: XSS ok. ?> data-show-on="<?php echo esc_attr( join( ' ', $show_on_devices ) ); ?>">
								<div class="<?php echo join( ' ', $inner_class ); ?>">
									<div class="container">
										<?php
										if ( $html_desktop_escaped ) {
											$c = 'cb-row--desktop hide-on-mobile hide-on-tablet';
											echo '<div class="row  ' . esc_attr( $c . ' ' . $align_classes ) . '">';
											echo $html_desktop_escaped; // WPCS: XSS ok.
											echo '</div>';
										}

										if ( $html_mobile_escaped ) {
											echo '<div class="cb-row--mobile hide-on-desktop row ' . esc_attr( $align_classes ) . '">';
	...
	public function render_mobile_sidebar() {
		...
		$menu_sidebar_skin = Qiupid()->get_setting( 'header_sidebar_skin_mode' );
		...
		if ( ! empty( $mobile_items ) || is_customize_preview() ) {
			...
			foreach ( $mobile_items as $row_id => $col_items ) {
				foreach ( $col_items as $item_index => $item ) {
					...
					echo $content_escaped; // WPCS: XSS ok.
					if ( is_customize_preview() ) {
...
/**
 * Alias of class Qiupid_Layout_Builder_Frontend_V2
 *
 * @see Qiupid_Layout_Builder_Frontend
 *
 * @return Qiupid_Layout_Builder_Frontend_V2
 */
function Qiupid_Layout_Builder_Frontend_V2() {
	return Qiupid_Layout_Builder_Frontend_V2::get_instance();
}
