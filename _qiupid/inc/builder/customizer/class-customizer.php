<?php
/**
 * Qiupid Theme Customizer
 *
 * @package qiupid
 */
class  Qiupid_Customizer {
	static $config;
	static $_instance;
	...
	function init() {
		...
		if ( is_admin() || is_customize_preview() ) {
			...
			add_action( 'wp_ajax_qiupid/customizer/ajax/get_icons', array( $this, 'get_icons' ) );
		...
		add_action( 'wp_ajax_qiupid__reset_section', array( 'Qiupid_Customizer', 'reset_customize_section' ) );
	}

	/**
	 * Instance.
	 *
	 * @return Qiupid_Customizer
	 */
	...
	function get_icons() {
		...
		wp_send_json_success( Qiupid_Font_Icons::get_instance()->get_icons() );
		die();
	}
	...
	function preview_js() {
		if ( is_customize_preview() ) {
			...
			wp_enqueue_script( 'qiupid-customizer-auto-css', esc_url( get_template_directory_uri() ) . '/assets/js/customizer/auto-css' . $suffix . '.js', array( 'customize-preview' ), '20151215', true );
			wp_enqueue_script(
				'qiupid-customizer',
			...
			wp_localize_script(
				'qiupid-customizer-auto-css',
				'Qiupid_Preview_Config',
				array(
	...
	/**
	 * Get all customizer settings/control that added via `qiupid/customizer/config` hook
	 *
	...
	static function get_config( $wp_customize = null ) {
		if ( is_null( self::$config ) ) {
			...
			$_config = apply_filters( 'qiupid/customizer/config', array(), $wp_customize );
			$config  = array();
			foreach ( $_config as $f ) {
				...
				$f = wp_parse_args(
					...
					array(
						 ...
						 * For settings
						 */
						'sanitize_callback'    => array( 'Qiupid_Sanitize_Input', 'sanitize_customizer_input' ),
	...
	function get_setting( $name, $device = 'desktop', $key = false ) {
		...
		if ( isset( $config[ 'setting|' . $name ] ) ) {
			...
			$default = apply_filters( 'qiupid/customize/settings-default', $default, $name );
	...
	function get_typo_fields() {
		$typo_fields = array(
			array(
				'name'    => 'font',
				'type'    => 'select',
				'label'   => esc_html__( 'Font Family', 'qiupid' ),
				'choices' => array(),
			),
			array(
				'name'    => 'font_weight',
				'type'    => 'select',
				'label'   => esc_html__( 'Font Weight', 'qiupid' ),
				'choices' => array(),
			),
			array(
				'name'  => 'languages',
				'type'  => 'checkboxes',
				'label' => esc_html__( 'Font Languages', 'qiupid' ),
			),
			array(
				'name'            => 'font_size',
				'type'            => 'slider',
				'label'           => esc_html__( 'Font Size', 'qiupid' ),
			...
			array(
				'name'            => 'line_height',
				'type'            => 'slider',
				'label'           => esc_html__( 'Line Height', 'qiupid' ),
				'device_settings' => true,
				'min'             => 9,
				'max'             => 80,
				'step'            => 1,
			...
			array(
				'name'  => 'letter_spacing',
				'type'  => 'slider',
				'label' => esc_html__( 'Letter Spacing', 'qiupid' ),
				'min'   => - 10,
				'max'   => 10,
				'step'  => 0.1,
			),
			array(
				'name'    => 'style',
				'type'    => 'select',
				'label'   => esc_html__( 'Font Style', 'qiupid' ),
				'choices' => array(
					''        => esc_html__( 'Default', 'qiupid' ),
					'normal'  => esc_html__( 'Normal', 'qiupid' ),
					'italic'  => esc_html__( 'Italic', 'qiupid' ),
					'oblique' => esc_html__( 'Oblique', 'qiupid' ),
				),
			),
			array(
				'name'    => 'text_decoration',
				'type'    => 'select',
				'label'   => esc_html__( 'Text Decoration', 'qiupid' ),
				'choices' => array(
					''             => esc_html__( 'Default', 'qiupid' ),
					'underline'    => esc_html__( 'Underline', 'qiupid' ),
					'overline'     => esc_html__( 'Overline', 'qiupid' ),
					'line-through' => esc_html__( 'Line through', 'qiupid' ),
			...
			array(
				'name'    => 'text_transform',
				'type'    => 'select',
				'label'   => esc_html__( 'Text Transform', 'qiupid' ),
				'choices' => array(
					''           => esc_html__( 'Default', 'qiupid' ),
					'uppercase'  => esc_html__( 'Uppercase', 'qiupid' ),
					'lowercase'  => esc_html__( 'Lowercase', 'qiupid' ),
					'capitalize' => esc_html__( 'Capitalize', 'qiupid' ),
					'none'       => esc_html__( 'None', 'qiupid' ),
				),
			),
		);
	...
	function get_styling_config() {
		$fields = array(
			'tabs'          => array(
				'normal' => esc_html__( 'Normal', 'qiupid' ),  // null or false to disable.
				'hover'  => esc_html__( 'Hover', 'qiupid' ), // null or false to disable.
			),
			'normal_fields' => array(
				array(
					'name'       => 'text_color',
					'type'       => 'color',
					'label'      => esc_html__( 'Color', 'qiupid' ),
					'css_format' => 'color: {{value}}; text-decoration-color: {{value}};',
				),
				array(
					'name'       => 'link_color',
					'type'       => 'color',
					'label'      => esc_html__( 'Link Color', 'qiupid' ),
					'css_format' => 'color: {{value}}; text-decoration-color: {{value}};',
				),
				...
				array(
					...
					'label'           => esc_html__( 'Margin', 'qiupid' ),
				),
				...
				array(
					'name'  => 'bg_heading',
					'type'  => 'heading',
					'label' => esc_html__( 'Background', 'qiupid' ),
				),
				...
				array(
					'name'       => 'bg_image',
					'type'       => 'image',
					'label'      => esc_html__( 'Background Image', 'qiupid' ),
				...
				array(
					...
					'choices'    => array(
						''        => esc_html__( 'Default', 'qiupid' ),
						'auto'    => esc_html__( 'Auto', 'qiupid' ),
						'cover'   => esc_html__( 'Cover', 'qiupid' ),
						'contain' => esc_html__( 'Contain', 'qiupid' ),
					),
				...
				array(
					'name'       => 'bg_position',
					'type'       => 'select',
					'label'      => esc_html__( 'Position', 'qiupid' ),
					'required'   => array( 'bg_image', 'not_empty', '' ),
					'class'      => 'field-half-right',
					'choices'    => array(
						''              => esc_html__( 'Default', 'qiupid' ),
						'center'        => esc_html__( 'Center', 'qiupid' ),
						'top left'      => esc_html__( 'Top Left', 'qiupid' ),
						'top right'     => esc_html__( 'Top Right', 'qiupid' ),
						'top center'    => esc_html__( 'Top Center', 'qiupid' ),
						'bottom left'   => esc_html__( 'Bottom Left', 'qiupid' ),
						'bottom center' => esc_html__( 'Bottom Center', 'qiupid' ),
						'bottom right'  => esc_html__( 'Bottom Right', 'qiupid' ),
					),
				...
				array(
					'name'       => 'bg_repeat',
					'type'       => 'select',
					'label'      => esc_html__( 'Repeat', 'qiupid' ),
					'class'      => 'field-half-left',
					'required'   => array(
						array( 'bg_image', 'not_empty', '' ),
					),
					'choices'    => array(
						'repeat'    => esc_html__( 'Default', 'qiupid' ),
						'no-repeat' => esc_html__( 'No repeat', 'qiupid' ),
						'repeat-x'  => esc_html__( 'Repeat horizontal', 'qiupid' ),
						'repeat-y'  => esc_html__( 'Repeat vertical', 'qiupid' ),
					),
				...
				array(
					'name'       => 'bg_attachment',
					'type'       => 'select',
					'label'      => esc_html__( 'Attachment', 'qiupid' ),
					'class'      => 'field-half-right',
					'required'   => array(
						array( 'bg_image', 'not_empty', '' ),
					),
					'choices'    => array(
						''       => esc_html__( 'Default', 'qiupid' ),
						'scroll' => esc_html__( 'Scroll', 'qiupid' ),
						'fixed'  => esc_html__( 'Fixed', 'qiupid' ),
					),
				...
				array(
					'name'  => 'border_heading',
					'type'  => 'heading',
					'label' => esc_html__( 'Border', 'qiupid' ),
				),
				...
				array(
					'name'       => 'box_shadow',
					'type'       => 'shadow',
					'label'      => esc_html__( 'Box Shadow', 'qiupid' ),
		...
		return apply_filters( 'qiupid/get_styling_config', $fields );
	}
	...
	function load_controls() {
		...
		$fields = apply_filters( 'qiupid/customize/register-controls', $fields );
		...
		foreach ( $fields as $k => $f ) {
			...
			if ( file_exists( $file ) ) {
				...
				$control_class_name = 'Qiupid_Customizer_Control_';
	...
	function register( $wp_customize ) {
		...
		$wp_customize->register_panel_type( 'Qiupid_WP_Customize_Panel' );
		$wp_customize->register_section_type( 'Qiupid_WP_Customize_Section' );
		...
		foreach ( self::get_config( $wp_customize ) as $args ) {
			switch ( $args['type'] ) {
				case 'panel':
					...
					unset( $args['name'] );
					unset( $args['type'] );
					$panel = new Qiupid_WP_Customize_Panel( $wp_customize, $name, $args );
				...
				case 'section':
					...
					if ( isset( $args['section_class'] ) && class_exists( $args['section_class'] ) ) { // Allow custom class.
						...
						$wp_customize->add_section( new Qiupid_WP_Customize_Section( $wp_customize, $name, $args ) );
					}
				...
				default:
					...
					$settings_args['default'] = apply_filters( 'qiupid/customize/settings-default', $args['default'], $args['name'] );

					$settings_args['transport'] = 'refresh';
					if ( ! $settings_args['sanitize_callback'] ) {
						$settings_args['sanitize_callback'] = array(
							'Qiupid_Sanitize_Input',
					...
					if ( 'js_raw' != $settings_args['type'] ) {
						if ( class_exists( $control_class_name ) ) {
							...
							$wp_customize->add_control( new Qiupid_Customizer_Control_Base( $wp_customize, $name, $args ) );
						}
					}
					...
					if ( $selective_refresh ) {
						...
						if ( ! isset( $this->selective_settings[ $__id ] ) ) {
							$this->selective_settings[ $__id ] = array(
								...
								'container_inclusive' => ( strpos( $__id, 'Qiupid_Customizer_Auto_CSS' ) === false ) ? true : false,
		...
		$wp_customize->get_section( 'title_tagline' )->title        = esc_html__( 'Logo', 'qiupid' );
		...
		foreach ( $this->selective_settings as $cb => $settings ) {
			...
			if ( 'Qiupid_Builder_Item_Logo__render' == $cb ) {
		...
		$wp_customize->add_setting(
			...
			array(
				...
				'sanitize_callback' => array( 'Qiupid_Sanitize_Input', 'sanitize_css_code' ),
			)
		);

		do_action( 'qiupid/customize/register_completed', $this );
	}
