...
class Qiupid {
	...
	/**
	 * Main Qiupid Instance.
	 *
	 * Ensures only one instance of Qiupid is loaded or can be loaded.
	 *
	 * @return Qiupid Main instance.
	 */
	...
	function content_width() {
		$GLOBALS['content_width'] = apply_filters( 'qiupid_content_width', 843 );
	}
	...
	function scripts() {
		...
		if ( ! class_exists( 'Qiupid_Font_Icons' ) ) {
			require_once get_template_directory() . '/inc/builder/customizer/class-customizer-icons.php';
		}
		Qiupid_Font_Icons::get_instance()->enqueue();

		$suffix = $this->get_asset_suffix();

		do_action( 'qiupid/load-scripts' );

		$css_files = apply_filters(
			'qiupid/theme/css',
			array(
				'google-font' => Qiupid_Customizer_Auto_CSS::get_instance()->get_font_url(),
		...
		foreach ( $css_files as $id => $url ) {
			...
			if ( is_array( $url ) ) {
				...
				wp_enqueue_style( 'qiupid-' . $id, $arg['url'], $arg['deps'], $arg['ver'] );
			} elseif ( $url ) {
				wp_enqueue_style( 'qiupid-' . $id, $url, $deps, self::$version );
			}
		}
		...
		wp_add_inline_style( 'qiupid-style', Qiupid_Customizer_Auto_CSS::get_instance()->auto_css() );
		wp_localize_script(
			'qiupid-themejs',
			'Qiupid_JS',
			apply_filters( // phpcs:ignore
				...
				array(
					'is_rtl'                     => is_rtl(),
					'css_media_queries'          => Qiupid_Customizer_Auto_CSS::get_instance()->media_queries,
					'sidebar_menu_no_duplicator' => Qiupid()->get_setting( 'header_sidebar_menu_no_duplicator' ),
				)
			)
		);

		do_action( 'qiupid/theme/scripts' );
	}
	...
	function init() {
		$this->init_hooks();
		$this->includes();
		$this->customizer = Qiupid_Customizer::get_instance();
		$this->customizer->init();
		do_action( 'qiupid/init' );
	}

	function get_setting( $id, $device = 'desktop', $key = null ) {
		return Qiupid_Customizer::get_instance()->get_setting( $id, $device, $key );
	}

	function get_media( $value, $size = null ) {
		return Qiupid_Customizer::get_instance()->get_media( $value, $size );
	}

	function get_setting_tab( $name, $tab = null ) {
		return Qiupid_Customizer::get_instance()->get_setting_tab( $name, $tab );
	}
