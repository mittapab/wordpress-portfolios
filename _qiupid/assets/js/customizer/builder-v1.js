		...
		var Builder = {
			...
			drag_drop: function() {
				...
				$(".qiupid--device-panel", that.container).each(function() {
					...
					var sortable_ids = [];
					that.panels[device] = {};
					$(".qiupid--cb-items", panel).each(function(index) {
					...
					$(
						".qiupid-available-items .grid-stack-item",
						panel
					).draggable({
						revert: "invalid",
						...
						start: function(event, ui) {
							...
							$(".qiupid--cb-items", panel).css("z-index", "");
			...
			addNewWidget: function($item, row) {
				...
				var panel = that.container.find(
					".qiupid--device-panel.qiupid--panel-" +
						that.activePanel
				);
				var el = row;
				if (!_.isObject(el)) {
					el = panel.find(".qiupid--cb-items").first();
				}
			...
			addPanel: function(device) {
				...
				var template_id = "tmpl-qiupid--cb-panel";
			...
			addAvailableItems: function() {
				...
				_.each(that.devices, function(device_name, device) {
					var $itemWrapper = $(
						'<div class="qiupid-available-items" data-device="' +
							device +
							'"></div>'
					);
					...
					_.each(device_data, function(items, row_id) {
						if (!_.isUndefined(items)) {
							_.each(items, function(node, index) {
								var item = $(
									'.qiupid-available-items[data-device="' +
										device +
