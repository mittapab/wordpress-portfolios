...
(function($, api) {
	...
	var header_changed = function(partial_id, remove_items) {
		...
		if (
			...
			partial_id === "qiupid_customize_render_header"
		) {
	...
	api.bind("preview-ready", function() {
		...
		function setupPreviewNamePosition() {
			$(".qiupid-grid .has_menu.builder-item-focus").each(function() {
				var parentPos = $(this)
					.closest(".qiupid-grid")
					.offset();
	...
	var skips_to_add_shortcut = {
		qiupid_customize_render_header: 1,
		qiupid_customize_render_footer: 1
	};
