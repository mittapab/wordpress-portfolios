 ...
 * @since 0.2.9
 */
class Qiupid_Layout_Builder_Frontend extends Qiupid_Abstract_Layout_Frontend {
	...
	function get_settings() {
		...
		$data = Qiupid()->get_setting( $this->control_id );
		$data = wp_parse_args(
			$data,
			array(
				'desktop' => '',
				...
				if ( is_array( $row_items ) && count( $row_items ) ) {
					...
					foreach ( $row_items as $item_index => $item ) {
						...
						if ( ! $items[ $item['id'] ] ['render_content'] ) {
							 ...
							 * @since 0.2.1
							 */
							do_action( 'qiupid/builder/' . $this->id . '/before-item/' . $item['id'] );
							$object_item = Qiupid_Customize_Layout_Builder()->get_builder_item( $this->id, $item['id'] );
							 ...
							 * @since 0.2.1
							 */
							do_action( 'qiupid/builder/' . $this->id . '/after-item/' . $item['id'] );
	...
	public function render_row( $items, $id = '', $device = 'desktop' ) {
		...
		while ( $index < $n ) {
			...
			$merge      = Qiupid()->get_setting( $merge_key, $device );
			$merge_next = false;
			$merge_prev = false;
			if ( 'no' == $merge || '0' == $merge ) {
				$merge = false;
			...
			if ( $next_item ) {
				...
				$merge_next     = Qiupid()->get_setting( $merge_key_next, $device );
			}
		...
		foreach ( $group_items as $item ) {
			...
			$classes = apply_filters( 'qiupid/builder/item-wrapper-classes', $classes, $item );
			...
			foreach ( $item['items'] as $_it ) {
				...
				if ( $content ) {
					...
					$row_items_html_escaped .= '<div class="' . esc_attr( $item_classes ) . '" data-section="' . $item_config['section'] . '" data-item-id="' . esc_attr( $item_id ) . '" >';
 ...
 * @deprecated 0.2.7
 */
class Qiupid_Customize_Layout_Builder_Frontend extends Qiupid_Layout_Builder_Frontend {}

/**
 * Alias of class Qiupid_Layout_Builder_Frontend
 *
 * @see Qiupid_Layout_Builder_Frontend
 *
 * @return Qiupid_Layout_Builder_Frontend
 */
function Qiupid_Layout_Builder_Frontend() {
	return Qiupid_Layout_Builder_Frontend::get_instance();
}
 ...
 * @return Qiupid_Layout_Builder_Frontend
 */
function Qiupid_Customize_Layout_Builder_Frontend() {
	return Qiupid_Layout_Builder_Frontend();
}
