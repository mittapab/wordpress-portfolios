...
class Qiupid_Customizer_Control_Base extends WP_Customize_Control {
	/**
	 * The control type.
	 *
	 * @access public
	...
	public function to_json() {
		...
		$this->json['_pro']           = class_exists( 'Qiupid_Pro' );
		...
		if ( 'repeater' == $this->setting_type ) {
			$this->json['l10n']             = array(
				'untitled' => esc_html__( 'Untitled', 'qiupid' ),
			);
	...
	function merge_items( $destination = array(), $new_array = array() ) {
		return Qiupid_Customizer::merge_items( $destination, $new_array );
	}
	...
	public function enqueue() {
		...
		wp_enqueue_style( 'qiupid-customizer-control', esc_url( get_template_directory_uri() ) . '/assets/css/admin/customizer/customizer' . $suffix . '.css' ); // phpcs:ignore
		wp_enqueue_style( 'select2', esc_url( get_template_directory_uri() ) . '/assets/css/admin/select2' . $suffix . '.css' ); // phpcs:ignore
		wp_enqueue_script( 'qiupid-color-picker-alpha', esc_url( get_template_directory_uri() ) . '/assets/js/customizer/color-picker-alpha' . $suffix . '.js', array( 'wp-color-picker' ), false, true ); // phpcs:ignore
		...
		if ( is_null( self::$_args_loaded ) ) {
			...
			$args = array(
				...
				'is_rtl'           => is_rtl(),
				'theme_default'    => esc_html__( 'Theme Default', 'qiupid' ),
				'reset'            => esc_html__( 'Reset this section settings', 'qiupid' ),
				'untitled'         => esc_html__( 'Untitled', 'qiupid' ),
				'confirm_reset'    => esc_html__( 'Do you want to reset this section settings?', 'qiupid' ),
				'list_font_weight' => array(
					''       => esc_html__( 'Default', 'qiupid' ),
					'normal' => _x( 'Normal', 'qiupid-font-weight', 'qiupid' ),
					'bold'   => _x( 'Bold', 'qiupid-font-weight', 'qiupid' ),
				),
				'typo_fields'      => Qiupid()->customizer->get_typo_fields(),
				'styling_config'   => Qiupid()->customizer->get_styling_config(),
				'devices'          => Qiupid()->customizer->devices,
			...
			wp_localize_script( 'qiupid-customizer-control', 'Qiupid_Control_Args', apply_filters( 'Qiupid_Control_Args', $args ) ); // phpcs:ignore

			$color_picker_l10n_args = array(
				'clear'            => esc_html__( 'Clear', 'qiupid' ),
				'clearAriaLabel'   => esc_html__( 'Clear color', 'qiupid' ),
				'defaultString'    => esc_html__( 'Default', 'qiupid' ),
				'defaultAriaLabel' => esc_html__( 'Select default color', 'qiupid' ),
				'pick'             => esc_html__( 'Select Color', 'qiupid' ),
				'defaultLabel'     => esc_html__( 'Color value', 'qiupid' ),
			);
			wp_localize_script( 'wp-color-picker', 'Qiupid_ColorPicker_L10n', apply_filters( 'Qiupid_Control_Color_Picker_L10n_Args', $color_picker_l10n_args ) ); // phpcs:ignore
	...
	protected function render() {
		...
		?>
		<li id="<?php echo esc_attr( $id ); ?>" class="<?php echo esc_attr( $class ); ?><?php echo esc_attr( $this->device ) ? '  qiupid--device-show qiupid--device-' . esc_attr( $this->device ) : ''; ?>">
	...
	protected function render_content() {
		...
		if ( 'heading' == $this->setting_type ) {
			?>
			<div class="qiupid-control--heading">
				<label>
		...
		?>
		<div class="qiupid--settings-wrapper <?php echo ( isset( $this->description ) && ! empty( $this->description ) ) ? 'show-short-desc' : ''; ?> <?php echo esc_attr( $this->label || $this->description ) ? 'has-title-desc' : 'no-title-desc'; ?>">
			<?php if ( $this->label ) { ?>
				<div data-control="<?php echo esc_attr( $this->id ); ?>" class="qiupid-control-field-header qiupid-field-heading">
					<label>
						<?php if ( ! empty( $this->label ) ) : ?>
							<span class="customize-control-title"><?php echo esc_html( $this->label ); ?></span>
						<?php endif; ?>
			...
			<?php if ( 'custom_html' != $this->setting_type ) { ?>
				<div class="qiupid--settings-fields<?php echo ( 'repeater' == $this->setting_type ) ? ' qiupid--repeater-items' : ''; ?>"></div>
				<?php if ( 'repeater' == $this->setting_type ) { ?>
					<div class="qiupid--repeater-actions">
						<a href="#" class="qiupid--repeater-reorder" data-text="<?php esc_attr_e( 'Reorder', 'qiupid' ); ?>" data-done="<?php _e( 'Done', 'qiupid' ); ?>"><?php _e( 'Reorder', 'qiupid' ); ?></a>
						<?php if ( false !== $this->addable ) { ?>
							<button type="button" class="button qiupid--repeater-add-new"><?php _e( 'Add an item', 'qiupid' ); ?></button>
						<?php } ?>
					</div>
				<?php } ?>
			<?php } ?>
	...
	static function before_field() {
		...
		#>
		<div class="qiupid--field qiupid--field-{{ field.type }} {{ field.class }} qiupid--field-name-{{ field.original_name }}" data-required="{{ required }}" data-field-name="{{ field.name }}">
		<?php
	}
	...
	static function field_header() {
		...
		<div class="qiupid-field-header">
			<# if ( field.label ) { #>
			<div class="qiupid-field-heading">
	...
	static function field_template() {
		?>
		<script type="text/html" id="tmpl-field-qiupid-text">
			<?php
			self::before_field();
			?>
			<?php echo self::field_header(); ?>
			<div class="qiupid-field-settings-inner">
				<input type="{{ field.type }}" class="qiupid-input qiupid-only" placeholder="{{ field.placeholder }}" data-name="{{ field.name }}" value="{{ field.value }}">
			</div>
			<?php
			self::after_field();
			?>
