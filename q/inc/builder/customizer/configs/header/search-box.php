...
class Qiupid_Builder_Item_Search_Box {
	...
	function item() {
		return array(
			'name'    => esc_html__( 'Search Form', 'qiupid' ),
			'id'      => $this->id,
			'col'     => 0,
			'width'   => '1',
			'section' => 'search_box',
	...
	function customize() {
		...
		$config   = array(
			array(
				...
				'title' => esc_html__( 'Search Form', 'qiupid' ),
			),

			array(
				'name'            => 'qiupid_search_box_placeholder',
				'type'            => 'text',
				'section'         => 'search_box',
				'selector'        => "$selector",
				'render_callback' => $fn,
				'label'           => esc_html__( 'Placeholder', 'qiupid' ),
				'default'         => esc_html__( 'Search in...', 'qiupid' ),
				'priority'        => 10,
			),
			array(
				'name'            => 'qiupid_search_box_scope',
				'type'            => 'select',
				'section'         => 'search_box',
				'default'         => 'post',
			...
			array(
				'name'            => 'qiupid_search_box_icon_size',
				'type'            => 'slider',
				'device_settings' => true,
				'section'         => 'search_box',
				'min'             => 5,
			...
			array(
				'name'            => 'qiupid_search_box_icon_pos',
				'type'            => 'slider',
				'device_settings' => true,
				'default'         => array(
					'desktop' => array(
			...
			array(
				'name'        => 'qiupid_search_box_font_size',
				'type'        => 'typography',
				'section'     => 'search_box',
				'selector'    => "$selector .search-form-fields",
			...
			array(
				'name'        => 'qiupid_search_box_input_styling',
				'type'        => 'styling',
				'section'     => 'search_box',
				'css_format'  => 'styling',
			...
			array(
				'name'        => 'qiupid_search_box_icon_styling',
				'type'        => 'styling',
				'section'     => 'search_box',
				'css_format'  => 'styling',
		...
		// Item Layout.
		return array_merge( $config, qiupid_header_layout_settings( $this->id, 'search_box' ) );
	}
	 ...
	 */
	function render() {
		$form_extra_class = apply_filters( 'qiupid/builder_item/search-box/form_extra_class', array() );
		$scope = Qiupid()->get_setting( 'qiupid_search_box_scope' );
		$placeholder = Qiupid()->get_setting( 'qiupid_search_box_placeholder' );
		$placeholder = sanitize_text_field( $placeholder );

		do_action( 'qiupid/builder_item/search-box/before_html' );
		...
		<form role="search" class="header-search-form <?php echo esc_attr( implode( ' ', $form_extra_class ) ); ?>" action="<?php echo esc_url( home_url( '/' ) ); ?>">
			<div class="search-form-fields">
				...
				<?php
				do_action( 'qiupid/builder_item/search-box/html_content/before_input' );
				do_action('qiupid_search_form_before');
				?>

				<input type="hidden" value="<?php echo esc_attr($scope); ?>" name="post_type" />
				<input type="search" autocomplete="off" class="search-field" placeholder="<?php echo esc_attr( $placeholder ); ?>" value="<?php echo get_search_query(); ?>" name="s" title="<?php echo esc_attr_x( 'Search for:', 'label', 'qiupid' ); ?>" />

				<?php
				do_action( 'qiupid/builder_item/search-box/html_content/after_input' );
				?>
			</div>
		...
		<?php
		echo '</div>';
		do_action( 'qiupid/builder_item/search-box/after_html' );
	}
}

Qiupid_Customize_Layout_Builder()->register_item( 'header', new Qiupid_Builder_Item_Search_Box() );
