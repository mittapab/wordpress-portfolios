...
class Qiupid_Builder_Header extends Qiupid_Customize_Builder_Panel {
	public $id = 'header';
	...
	function get_config() {
		return array(
			'id'            => $this->id,
			'title'         => esc_html__( 'Header Builder', 'qiupid' ),
			...
			'versions'      => array(
				'v1' => array(
					...
					'label' => esc_html__( 'Version 1', 'qiupid' ),
				),
				'v2' => array(
					'control_id' => 'header_builder_panel_v2',
					'label' => esc_html__( 'Version 2', 'qiupid' ),
				),
			),
			'devices'       => array(
				'desktop'      => esc_html__( 'Desktop', 'qiupid' ),
				'mobile'       => esc_html__( 'Mobile/Tablet', 'qiupid' ),
			),
		);
	}
	...
	function get_rows_config() {
		return array(
			'top'     => esc_html__( 'Header Top', 'qiupid' ),
			'main'    => esc_html__( 'Header Main', 'qiupid' ),
			'bottom'  => esc_html__( 'Header Bottom', 'qiupid' ),
			'sidebar' => esc_html__( 'Menu Sidebar', 'qiupid' ),
		);
	}
	...
	function row_config( $section = false, $section_name = false ) {
		...
		if ( ! $section_name ) {
			$section_name = esc_html__( 'Header Top', 'qiupid' );
		}
		...
		$config = array(
			...
			array(
				...
				'title'           => esc_html__( 'Container Width', 'qiupid' ),
			),
			array(
				...
				'title'           => esc_html__( 'Height', 'qiupid' ),
			),
			...
			array(
				...
				'type'             => 'styling',
				'section'          => $section,
				'title'            => esc_html__( 'Advanced Styling', 'qiupid' ),
				'description'      => sprintf( __( 'Advanced styling for %s', 'qiupid' ), $section_name ),
			...
			array(
				...
				'section'          => $section,
				'default'          => '1',
				'title'            => esc_html__( 'Visible on Sticky Header?', 'qiupid' ),
				'checkbox_label' => esc_html__( 'Show/Hide', 'qiupid' ),
				'description'      => esc_html__( '*If sticky header is enabled', 'qiupid' ),
...
if ( ! function_exists( 'qiupid_header_layout_settings' ) ) {
	function qiupid_header_layout_settings( $item_id = '', $section = '', $cb = '', $name_prefix = 'header_' ) {

		if ( ! $cb ) {
			$cb = 'qiupid_customize_render_header';
		}
		...
		if ( ! $name_prefix ) {
			...
			if ( strpos( $item_id, 'footer' ) !== false ) {
				$class       = '.footer--row';
				$name_prefix = 'footer_';
				$cb          = 'qiupid_customize_render_footer';
			}
		}
		...
		$layout = array(
			array(
				...
				'priority' => 800,
				'section'  => $section,
				'title'    => esc_html__( 'Item Layout', 'qiupid' ),
			),
...
Qiupid_Customize_Layout_Builder()->register_builder( 'header', new Qiupid_Builder_Header() );
