(function($, wpcustomize) {
	...
	wpcustomize.bind("ready", function(e, b) {
		var addVersionChange = function(opts, builder, id, version) {
			if ( Qiupid_Layout_Builder.hide_switcher === 'yes' ) {
				return ;
			}
			...
			if (typeof opts.versions === "object") {
				...
				if (select_options) {
					var $select_options = $(
						'<span class="qiupid-swicth-version button-primary"><span>'+Qiupid_Layout_Builder.swicth_version+'</span><select class="builder-version-change ">' +
							select_options +
							"</select></span>"
					);
					$(".qiupid--cb-actions", builder.container).prepend(
						$select_options
					);
				}
				...
				$select_options.on("change", 'select', function() {
					var newVer = $(this).val();
					if ( qiupidPanels[id + version] ) {
						qiupidPanels[id + version].container.hide();
					}
					
					if (typeof qiupidPanels[id + newVer] === "undefined") {
						var _builder;
						if ("v2" === newVer) {
						...
						qiupidPanels[id + newVer] = _builder;
					}
					...
					if ( qiupidPanels[id + newVer] ) {
						qiupidPanels[id + newVer].container.show();
					}
		...
		_.each(Qiupid_Layout_Builder.builders, function(opts, id) {
			var builder;
			var version = "v1";
			...
			qiupidPanels[id + version] = builder;
				...
				if (
					...
					$(
						...
						'<p class="footer_moved_widgets_text">' +
							Qiupid_Layout_Builder.footer_moved_widgets_text +
							"</p>"
					);
				}
			}, 1000);
		...
		wpcustomize.state("expandedSection").bind(function(section) {
			$(".qiupid--device-panel .grid-stack-item").removeClass(
				"item-active"
			);
			$(".qiupid--cb-row").removeClass("row-active");
			if (section) {
				$('.qiupid--cb-row[data-id="' + section.id + '"]').addClass(
					"row-active"
				);
				$(
					".qiupid--device-panel .grid-stack-item.for-s-" +
						section.id
		...
		function(e) {
			...
			if (template_name && template_name !== "") {
				...
				$.post(
					ajaxurl,
					{
						action: "qiupid_builder_save_template",
						name: input.val(),
	...
	$document.on("mouseover", ".qiupid--cb-row .grid-stack-item:not(.no-tooltip)", function(
		e
	) {
		var item = $(this);
		var nameW =
			$(".qiupid--cb-item-remove", item).outerWidth() +
			$(".qiupid--cb-item-setting", item).outerWidth();
	...
	$document.on("mouseleave", ".qiupid--cb-row .grid-stack-item:not(.no-tooltip)", function(
		e
	) {
