...
(function($) {
	...
	var is_rtl = Qiupid_Layout_Builder.is_rtl;
	...
	CustomizeBuilder_V2 = function(options, id) {
		var Builder = {
			...
			drag_drop: function() {
				...
				$(".qiupid--device-panel", that.container).each(function() {
					var panel = $(this);
					...
					$(".col-items, .qiupid-available-items", panel).each(function() {
			...
			addPanel: function(device) {
				...
				var template_id = "tmpl-qiupid--cb-panel-v2";
				...
				return (
					'<div class="qiupid--device-panel qiupid-vertical-panel qiupid--panel-' +
					device +
					'" data-device="' +
					device +
					'">' +
			...
			addDevicePanels: function() {
				...
				_.each(that.devices, function(device_name, device) {
					...
					$(".qiupid--cb-devices-switcher", that.container).append(
					...
					$(".qiupid--cb-body", that.container).append(panelHTML);
				});

				if ($("#qiupid-upsell-tmpl").length) {
					$($("#qiupid-upsell-tmpl").html()).insertAfter(
						$(".qiupid--cb-devices-switcher", that.container)
					);
				}
			},
			addItem: function(node) {
				var that = this;
				var template = that.getTemplate();
				var template_id = "tmpl-qiupid--cb-item";
				if ($("#" + template_id).length == 0) {
					return;
				}
			...
			addAvailableItems: function() {
				...
				_.each(that.devices, function(device_name, device) {
					var $itemWrapper = $(
						'<div class="qiupid-available-items" data-device="' + device + '"></div>'
					);

					$(".qiupid--panel-" + device, that.container).append(
						$itemWrapper
					);
			...
			switchToDevice: function(device, toggle_button) {
				...
				if (numberDevices > 1) {
					$(
						".qiupid--cb-devices-switcher a",
						that.container
					).removeClass("qiupid--tab-active");
					$(
						".qiupid--cb-devices-switcher .switch-to-" + device,
						that.container
					).addClass("qiupid--tab-active");
					$(".qiupid--device-panel", that.container).addClass(
						"qiupid--panel-hide"
					);
					$(
						".qiupid--device-panel.qiupid--panel-" + device,
						that.container
					).removeClass("qiupid--panel-hide");
			...
			addNewWidget: function( device, row_id, col_id, node, index) {
				...
				row = $( '.qiupid--cb-row.qiupid--row-'+row_id, panel );
				col = $( '.col-items.col-'+col_id, row );

				var $item = $( '.qiupid-available-items .grid-stack-item[data-id="'+node.id+'"]', panel );
			...
			focus: function() {
				this.container.on(
					"click",
					".qiupid--cb-item-setting, .qiupid--cb-item-name, .item-tooltip",
					function(e) {
						e.preventDefault();
			...
			remove: function() {
				...
				$document.on(
					"click",
					".qiupid--device-panel .qiupid--cb-item-remove",
					function(e) {
						e.preventDefault();
						var item = $(this).closest(".grid-stack-item");
						var panel = item.closest(".qiupid--device-panel");
						item.removeAttr("style");
						$(".qiupid-available-items", panel).append(item);
						that.save();
					}
				);
			},
			...
			save: function() {
				...
				_.each( that.devices, function( device_label, device ){
					data[ device ] = {};
					var devicePanel = $( '.qiupid--panel-'+device, that.container );
					$( '.qiupid--cb-row', devicePanel ).each( function(){
						var row = $( this );
			...
			showPanel: function() {
				var that = this;
				this.container
					.removeClass("qiupid--builder--hide")
					.addClass("qiupid--builder-show");
				setTimeout(function() {
			...
			hidePanel: function() {
				this.container.removeClass("qiupid--builder-show");
				$("#customize-preview")
			...
			togglePanel: function() {
				...
				wpcustomize.state("expandedPanel").bind(function(paneVisible) {
					if (wpcustomize.panel(options.panel).expanded()) {
						$document.trigger("qiupid_panel_builder_open", [
							options.panel
						]);
				...
				that.container.on("click", ".qiupid--panel-close", function(
					e
				) {
					e.preventDefault();
					that.container.toggleClass("qiupid--builder--hide");
					if (that.container.hasClass("qiupid--builder--hide")) {
			...
			init: function(controlId, items, devices) {
				...
				var template = that.getTemplate();
				var template_id = "tmpl-qiupid--builder-panel";
				var html = template(options, template_id);
				that.container = $(html);
				that.container.addClass( 'qiupid--panel-v2' );
				...
				if (options.section) {
					wpcustomize
						...
						.container.addClass("qiupid--hide");
				}
				...
				that.container.on(
					"click",
					".qiupid--cb-devices-switcher a.switch-to",
					function(e) {
						e.preventDefault();
				...
				$document.trigger("qiupid_builder_panel_loaded", [id, that]);
